angular.module('starter.controllers', ['ionic', 'ionic.contrib.frostedGlass', 'ionic.contrib.ui.hscrollcards',
    'ion-autocomplete', 'starter.services'])
    /** 出于演示方便，所有controller都放在了一起，实际产品 通常是分开各个文件*/
    .controller('AppCtrl', function ($scope, $ionicPopup, $ionicModal, $state, $timeout, $ionicNavBarDelegate, $ionicHistory, AUTH_EVENTS, AuthService) {
        ...
        $scope.doLogin = function (loginData) {
            AuthService.login(loginData.username, loginData.password).then(function (authenticated) {
                ...
                $state.go('app.authtest', {}, {reload: true});
    ...
    .controller('PlaylistsCtrl', function ($scope, PhoneService) {
    ...
    .controller('ChatsCtrl', function ($scope, Chats) {
        ...
        $scope.loadMore = function () {
            data = Chats.allnew($scope.chats.length);
            Array.prototype.push.apply($scope.chats, data);
    ...
    .controller('RateCtrl', ['$scope', '$ionicLoading', '$timeout', function ($scope, $ionicLoading, $timeout) {
    ...
    .controller('ListRefreshCtrl', function ($scope, DataFactory, $ionicScrollDelegate) {
        $scope.items = [];
        ...
        $scope.doRefresh = function () {
            DataFactory.getData(3)
                .then(function (data) {
                    ...
                    // 数组插入数组最优做法
                    Array.prototype.splice.apply($scope.items, [0, 0].concat(data));
                    console.log("sdfsdf");
                    //$scope.items = data.concat($scope.items);
    ...
    .controller('ActionSheet', function ($scope, $ionicActionSheet, $timeout, $ionicPopover, $ionicPopup) {
        $scope.data = {};
        $scope.state = {};
        $scope.error = {};
    ...
    .controller('ListOption', function ($scope, DataFactory) {
    ...
    .controller('SlideCtrl', function ($scope, $state, $ionicSlideBoxDelegate) {
        ...
        var countSlides = function () {
            ...
            console.log($scope.data.numViewableSlides + " 个可见幻灯片");
    ...
    .controller('SearchCtrl', function ($scope, $timeout, $ionicFilterBar) {
        ...
        function getItems() {
            ...
            for (var x = 1; x < 2000; x++) {
                items.push({text: '当前id是' + x + ' ，该id是 ' + (x % 2 === 0 ? '偶数' : '奇数') + ' 数字'});
            }
            $scope.items = items;
        }
    ...
    .controller('MessageCtrl', ['$scope', '$timeout', '$sce','$ionicFrostedDelegate', '$ionicScrollDelegate', '$rootScope',
        function ($scope, $timeout,$sce,$ionicFrostedDelegate, $ionicScrollDelegate, $rootScope) {
            ...
            var messageOptions = [
                ...
                {content: '<p>Gee wiz, this is somethign special.</p>'},
    ...
    .controller('AutoDividerCtrl', function($scope, $timeout, PersonService) {
        $scope.items = [];
    ...
    .controller('HScrollCtrl', function($scope) {

        $scope.items = [];

        for (var i = 0; i <= 5; i++) {
            var tmp = [
                {desc: 'The Ramones', image:'img/images1.jpg'},
                {desc: 'The Beatles', image:'img/images2.png'},
                {desc: 'Pink Floyd', image:'img/images3.jpg'},
                {desc: 'The Rolling Stones', image:'img/images4.jpg'},
                {desc: 'The Jimi Hendrix Experience', image:'img/images5.jpg'},
                {desc: 'Van Halen', image:'img/images6.jpg'}
            ];
    ...
    .controller('AuthTestCtrl', function ($scope, $state, $http, $ionicPopup, AuthService) {
    ...
    .controller('UrlQrcodeCtrl', function ($scope) {
        $scope.bar = window.location.href;
    })

    .controller('EchartCtrl', function ($scope, $interval, AjaxChartDataFactory) {
        var pageload = {
            name: 'page.load',
            datapoints: [
                { x: 2001, y: 1012 },
        ...
        //$scope.ajaxdata = [pageload];
        AjaxChartDataFactory.getData()
            .then(function (data) {
                /*todo: 这样的静态数据都无法正常显示出来*/
                $scope.ajaxdata = [pageload];
                //    [{
                //    name: '当前股指',
                //    datapoints: data
        ...
        //    AjaxChartDataFactory.getData()
        //        .then(function (data) {
        //            Array.prototype.push.apply($scope.ajaxdata[0]['datapoints'], data);
        //        });
        //}, 3000);

    })

    .controller('ChartjsCtrl', function ($scope, crisisServiceFactory) {
        crisisServiceFactory.getcrisisList().success(function (response) {
            $scope.hostilityData = response['不友好报道统计'];

            $scope.negativeOnClick = function (points, evt) {
            ...
            $scope.parealabels = ["Download Sales", "In-Store Sales", "Mail-Order Sales", "Tele Sales", "Corporate Sales"];
            $scope.pareadata = [300, 500, 100, 40, 120];
        });
    })
